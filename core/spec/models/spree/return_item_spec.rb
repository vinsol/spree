...
describe Spree::ReturnItem, :type => :model do
  ...
  describe '#process_inventory_unit!' do
    ...
    context 'stock should restock' do
      it { expect(subject).to eq(Spree::StockMovement.find_by(stock_item_id: stock_item.id, quantity: 1, originator: return_item.return_authorization)) }
    end
